...
// Normalize title for directory name matching
...
kitabData.forEach(kitab => {
  const normalized = normalizeTitle(kitab.title);
  titleMapping[normalized] = kitab.title;
});

// Directory path
const basePath = '/data/work/dev/daurahtafsir-makro/syarah_hadis/syarah_kitab_bukhari';

// Get all existing directories
const directories = fs.readdirSync(basePath, { withFileTypes: true })
  .filter(dirent => dirent.isDirectory() && dirent.name.startsWith('sahih_bukhari_kitab_'))
  .map(dirent => dirent.name);

console.log(`Found ${directories.length} kitab directories`);

// Process each directory
directories.forEach(dir => {
  // Extract the kitab name part (remove 'sahih_bukhari_kitab_' prefix)
  const kitabName = dir.replace('sahih_bukhari_kitab_', '');
  
  // Try to find a matching title
  let matchedTitle = null;
  
  // First try exact match
  for (const [normalized, original] of Object.entries(titleMapping)) {
    if (normalizeTitle(original) === kitabName) {
  ...
  if (!matchedTitle) {
    for (const kitab of kitabData) {
      const normalizedKitab = normalizeTitle(kitab.title);
      if (normalizedKitab.includes(kitabName) || kitabName.includes(normalizedKitab)) {
        matchedTitle = kitab.title;
        break;
      }
    }
  }
  ...
  if (matchedTitle) {
    console.log(`${dir} -> ${matchedTitle}`);
  } else {
    console.log(`${dir} -> No matching title found`);
  }
});