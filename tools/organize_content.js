...
// Create directory names based on JSON titles
const newDirectories = kitabData.map(kitab => {
  const dirName = `sahih_bukhari_${normalizeTitleForDirectory(kitab.title)}`;
  return {
    originalTitle: kitab.title,
    url: kitab.url,
    dirName: dirName
  };
});
...
newDirectories.forEach(kitab => {
  const sourceDir = path.join(sourceBasePath, kitab.dirName);
  const destDir = path.join(destBasePath, kitab.dirName);
  ...
  if (fs.existsSync(sourceDir)) {
    ...
    console.log(`Copied ${files.length} files from ${kitab.dirName}`);
  } else {
    // Create empty directory for new kitab
    fs.mkdirSync(destDir, { recursive: true });
    console.log(`Created empty directory for ${kitab.dirName}`);
  }
});
